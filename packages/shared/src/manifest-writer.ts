...
/** Parse a manifest JSON file without trusting disk contents. */
function parseOnDiskManifest(content: string): OnDiskManifest {
	const parsed: unknown = JSON.parse(content);
	if (!isRecord(parsed)) {
		throw new Error('Manifest is not an object');
	}
	return {
		consolidated: parsed.consolidated === true ? true : undefined,
		buildTool: typeof parsed.buildTool === 'string' ? parsed.buildTool : undefined,
		buildTime: typeof parsed.buildTime === 'number' ? parsed.buildTime : undefined,
	};
}
...
export class ManifestWriter {
	...
	async readManifests(): Promise<AssetManifest[]> {
		try {
			...
			const manifestFiles = files.filter(
				(f) => f.endsWith('.json') && f !== 'consolidated-*.json',
			);
	...
	async cleanupOldManifests(maxAge: number = 24 * 60 * 60 * 1000): Promise<void> {
		try {
			const files = await fs.readdir(this.manifestDir);
			const now = Date.now();
			const manifestsByTool = new Map<string, ManifestFileRecord[]>();
			...
			for (const file of files) {
				...
				try {
					...
					const manifest = parseOnDiskManifest(content);
			...
			const consolidatedFiles = files.filter(
				(f) => f.startsWith('consolidated-') && f.endsWith('.json'),
			);
	...
	async cleanupBeforeConsolidation(): Promise<void> {
		try {
			...
			for (const file of files) {
				if (!file.endsWith('.json') || file.startsWith('consolidated-')) continue;
			...
			for (const manifests of manifestsByTool.values()) {
				if (manifests.length <= 1) {
					continue;
				}
	 ...
	 */
	private mergeCspDirectives(
		directivesArray: Partial<CSPProcessorOptions>[],
	): Partial<CSPProcessorOptions> {
				...
				if (value !== undefined) {
					if (key === 'baseDirectives' && typeof value === 'object' && value !== null) {
						...
						if (merged[key] === undefined) {
							...
							for (const [directiveKey, directiveValue] of Object.entries(
								newBaseDirectives,
							)) {
								if (directiveValue !== undefined) {
									if (existingBaseDirectives[directiveKey] === undefined) {
										...
										Array.isArray(existingBaseDirectives[directiveKey]) &&
										Array.isArray(directiveValue)
									) {
										// Merge arrays, removing duplicates
										const existingArray = existingBaseDirectives[
											directiveKey
										] as unknown[];
										const newArray = directiveValue as unknown[];
										existingBaseDirectives[directiveKey] = [
